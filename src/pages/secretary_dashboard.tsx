...
import { Appointment } from '@/types';
  ...
  if (typeof window !== 'undefined') {
    ...
    const deleteAppointment = (id:any)=>{
      ...
      .then(function (response) {
        ...
        toast('Appointment was deleted SuccessFully', { hideProgressBar: true, autoClose: 2000, type: 'success' ,position:'top-right' })    
 ...
 <nav className="menu" tabIndex={0}>
    ...
    <header className="avatar">
      <Image alt="" src={logo} style={{borderRadius:'50%',width:'80px'}} />
      <h6>Welcome</h6>
      <h5 style={{color:'white'}}>{login}</h5>
    </header>
    <ul>
      <li tabIndex={0} className="icon-customers"><Link href="" style={{textDecoration:"none",color:"white"}}><span>Appointment</span></Link></li>
      <li tabIndex={0} className="icon-folder"><Link href="" style={{textDecoration:"none",color:"white"}}><span>Patient Record</span></Link></li>
      ...
      <tbody key={index}>
        <tr>
          ...
          <td>
            ...
            <Link href="" onClick={()=>alertAppointment(item._id)} className="edit" title="Alert Appointment" data-toggle="tooltip">  <i className="material-icons add_alert">&#xe003;</i><ToastContainer/></Link>
            <Link href="" onClick={()=>getIdAppointment(item._id)}  className="confirm" title="Confirm Appointment" data-toggle="tooltip"><i className="material-icons done_outline">&#xe92f;</i><ToastContainer/></Link>
            <Link href="" className="edit" title="Edit Appointment" data-toggle="tooltip"><i className="material-icons">&#xE254;</i></Link>
            <Link href="" onClick={()=>deleteAppointment(item._id)} className="delete" title="Delete Appointment" data-toggle="tooltip"><i className="material-icons">&#xE872;</i></Link>
          </td>
        </tr>
